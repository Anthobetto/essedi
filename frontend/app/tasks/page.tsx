...
export default function Tasks() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans">
            ...
            <div className="overflow-hidden rounder-xl border border-gray-100 bg-white">
                <div className="overflow-x-auto">
                        <table className="w-full border border-collapse">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50">
                                    <th className="text-left py-3 px-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Name</th>
                                    <th className="text-left py-3 px-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Status</th>
                                    <th className="text-left py-3 px-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Client</th>
                                    <th className="text-left py-3 px-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Project</th>
                                    <th className="text-left py-3 px-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                {tasks.length === 0 ? (
                                ...
                                tasks.map((task) => (
                                    <tr key={task.id}>
                                        <td className="text-left py-3 px-4 text-sm text-gray-600">{task.name}</td>
                                        <td className="text-left py-3 px-4 text-sm text-gray-600">{task.status}</td>
                                        <td className="text-left py-3 px-4 text-sm text-gray-600">{task.company_name}</td>
                                        <td className="text-left py-3 px-4 text-sm text-gray-600">{task.project_name}</td>
                                        <td className="text-left py-3 px-4 text-sm text-gray-600">{new Date(task.created_at).toLocaleDateString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
